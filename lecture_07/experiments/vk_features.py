...
session_scope = 'wall,friends'
request_delay = 0.7


def login_to_vk():
    auth_data_file = open(auth_data_path, 'r')

    app_id = auth_data_file.readline().strip()
    login = auth_data_file.readline().strip()
    password = auth_data_file.readline().strip()
